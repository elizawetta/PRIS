...
Задачи:
- Бизнес-логика (PO): Определение приоритетов. 
...
Также роли пользователей продемонстрированы в [Use Case диаграмме](diagrams.md#51-use-case-diagram)
...
Также [тут](diagrams.md#53-%D0%B4%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0-%D0%B0%D0%BA%D1%82%D0%B8%D0%B2%D0%BD%D0%BE%D1%81%D1%82%D0%B5%D0%B9-activity-diagram) представлена диаграмма акитвностей, которая показывает логику принятия решения (проверка аномалий).
...
Подробная инфологическая модель и связи между таблицами представлены в документе:
[ER-диаграмма структуры данных](diagrams.md#23-er-%D0%B4%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0)
...
[Тут](diagrams.md#32-%D0%B4%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0-%D0%B0%D1%80%D1%85%D0%B8%D1%82%D0%B5%D0%BA%D1%82%D1%83%D1%80%D1%8B) можно изучить диаграмму архитектуры, которая обосновывает использование Kafka, Spark и шардирование.
...
Для архитектуры решения была спроектирована [UML-диаграмма классов](diagrams.md#42-uml-%D0%B4%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0-%D0%BA%D0%BB%D0%B0%D1%81%D1%81%D0%BE%D0%B2), которая демонстрирует программную реализацию (Store, Product, Forecast).
...
Взаимодействие между сервисами построено на комбинации **событийной модели** (для сбора данных) и **REST-архитектуры** (для получения прогнозов). Это обеспечивает надежность при обработке 2,67 млрд транзакций.

**Интеграция слоя данных (Data Layer $\rightarrow$ Storage)**
*   **Метод передачи:** Пакетная запись (Batch Insert).
*   **Описание:** **Ingestion Service** собирает JSON-сообщения от POS-терминалов 500 магазинов. Чтобы не перегружать **ClickHouse** миллионами мелких запросов, сервис буферизирует данные и записывает их пачками по 100 000 строк каждые 5 минут. Это гарантирует высокую скорость записи и консистентность Big Data хранилища.

**Интеграция обучения и инференса (ML Engine $\rightarrow$ Inference API)**
*   **Метод передачи:** Реестр моделей (Model Registry / MLflow).
*   **Описание:** После завершения обучения в **ML Engine**, артефакт модели (веса и параметры) сохраняется в реестр с уникальной версией. **Inference API** автоматически подхватывает новую версию модели без перезагрузки сервиса (Hot Swap), что обеспечивает бесперебойную выдачу прогнозов.

**Спецификация взаимодействия через Inference API**
Для обеспечения интеграции со слоем приложений используются описанные эндпоинты:

| Взаимодействие | Метод и путь | Формат данных | Описание потока |
| :--- | :--- | :--- | :--- |
| **Запрос автозаказа** | `GET /v1/orders/generate` | `JSON` | **ERP Connector** запрашивает прогноз на завтра для всей сети. Система возвращает список: `store_id`, `product_id`, `forecast_qty`. |
| **Детализация для UI** | `GET /v1/forecast/details` | `JSON` | **Admin Dashboard** запрашивает прогноз для графика. Ответ содержит доверительные интервалы и факторы влияния (например: "рост на 20% из-за промо"). |
| **Обратная связь** | `POST /v1/feedback/manual` | `JSON` | Менеджер через **UI** отправляет правку заказа. Данные передаются в **ClickHouse** для анализа причин отклонения прогноза ИИ. |
| **Синхронизация признаков** | `POST /v1/features/refresh` | `Internal RPC` | Сигнальный метод от **Data Layer** к **Feature Store**, уведомляющий о том, что свежие продажи за вчера загружены и лаги пересчитаны. |

**Интеграция с бизнес-контуром (Inference API $\rightarrow$ ERP)**
*   **Протокол:** REST API / Webhooks.
*   **Логика:** Раз в сутки, после того как **ML Engine** завершает расчет прогноза, **Inference API** отправляет уведомление (Webhook) в **ERP Connector**. Тот, в свою очередь, инициирует выгрузку данных в основную ERP-систему сети. Передача осуществляется в формате XML или CSV, стандартном для ритейл-систем, что гарантирует автоматическое создание документов «Заказ поставщику» до открытия магазинов.

**Мониторинг интеграций**
Все взаимодействия между сервисами логируются. В случае, если **Inference API** недоступен более 30 секунд, **ERP Connector** автоматически переключается на «Бейзлайн-режим» (расчет по скользящему среднему), чтобы магазины не остались без заказов в случае технических сбоев ИИ-слоя.

Для демонстрации пошагового взаимодействия сервисов во времени, была спроектирована [диаграмма поселдовательности](diagrams.md#52-%D0%B4%D0%B8%D0%B0%D0%B3%D1%80%D0%B0%D0%BC%D0%BC%D0%B0-%D0%BF%D0%BE%D1%81%D0%BB%D0%B5%D0%B4%D0%BE%D0%B2%D0%B0%D1%82%D0%B5%D0%BB%D1%8C%D0%BD%D0%BE%D1%81%D1%82%D0%B8-sequence-diagram)
